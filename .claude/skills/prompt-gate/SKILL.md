...
### 第 1 段 · 反方质询（点化并进战场）
加载 `arena/readmes/ai.md`（反方）。**先查 `arena/attacks/` 撞库**，命中打「命中」标记——记进战报，**不进对话**。
**对空架子**：审讯式逐项点缺项（目标→约束→架构→验收）,一次只点一个最卡的,逼人类亲口补。对话只以**自然辩论**呈现：点洞、逼补,不递答案（不列候选/不括注补全）、不教规则;答案形状 / 最小反例是脑内内功,不示形于对话;话要干练,**一轮 ≤3 条高洞**。立论有结构 → 正常挑架构洞 / 逻辑洞 / 可证性。
**下风计**:人类答空 / 被碾穿 → 点一格下风（`⚖️ 人类 [①] 〇 〇`）,满 3 格让裁判出场;答上关键洞 → 认账清零。铁律：**AI 拟稿 / 代填 / 递候选 = 禁**；人类丢「你看着办」→ 只换问法继续审,拒答到底记「零豁免」。

### 第 2 段 · 裁判裁决
加载 `arena/readmes/judge.md`,判 `PASS(人类胜) / FAIL(人类败) / 废止`。裁判**不插话**:人类满 3 格下风或立论自然收束才出场,用 `---` 与辩场隔离成段,格式 `评委认为人方[…]/ AI 有理有据[…]/ 判:人类败·人类胜,贴出 [human.md/ai.md] 受罚段`：
